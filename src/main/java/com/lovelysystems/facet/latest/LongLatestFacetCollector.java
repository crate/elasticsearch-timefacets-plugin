...
public class LongLatestFacetCollector extends AbstractFacetCollector {
    ...
    protected void doSetNextReader(IndexReader reader, int docBase)
        ...
        keyFieldData = (LongFieldData) fieldDataCache.cache(keyDataType,
                reader, keyFieldName);
        aggregator.valueFieldData = (LongFieldData) fieldDataCache.cache(
    ...
    public static class Aggregator implements LongValueInDocProc {
        ...
        final ExtTLongObjectHashMap<LongInternalLatestFacet.Entry> entries = CacheRecycler
                .popLongObjectMap();
        ...
        public void onValue(int docId, long key) {
            LongInternalLatestFacet.Entry entry = entries.get(key);
            long ts = tsFieldData.longValue(docId);
            if (entry == null || entry.ts < ts) {
                long value = valueFieldData.longValue(docId);
                if (entry == null) {
                    entry = new LongInternalLatestFacet.Entry(ts, value);
    ...
    @Override
    public Facet facet() {
        LongInternalLatestFacet f = new LongInternalLatestFacet(facetName, size, start,
